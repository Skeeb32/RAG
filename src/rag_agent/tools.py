...
TOOL_SCHEMAS = [
    schema(
        "search_knowledge",
        "Search the knowledge base for explanations and procedures. "
        "Resolve follow-up references using conversation history in the query.",
        SearchArgs,
    ),
    schema(
        "lookup_service",
        "Look up the current local demo service catalog record: owner, "
    ...
    def lookup(self, arguments: dict) -> dict:
        ...
        try:
            with sqlite3.connect(
                ...
                row = db.execute(
                    "SELECT service,owner,tier,runbook FROM services WHERE service=?",
                    (args.service,),
                ).fetchone()
            if row is None:
