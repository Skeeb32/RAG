...
def retrieval_scores(sources: list[str], relevant: list[str]) -> dict:
    ...
    return {"recall_at_3": len(found) / len(set(relevant)),
...
def summarize(rows: list[dict]) -> dict:
    ...
    for key in ["recall_at_3", "reciprocal_rank", "latency_ms", "correctness", "groundedness",
                "relevance", "route_correct"]:
...
def main() -> None:
    ...
    configs = [json.loads((ROOT / "experiments" / f"{name}.json").read_text())
    ...
    for config in configs:
        ...
        for case in cases:
            row = {"configuration": config["name"], "case_id": case["id"],
                   "status": "completed", "judgment_status": "not_requested"}
            if args.mode == "retrieval" and (not case["relevant_sources"] or
                                             case.get("prior_questions")):
                row.update(status="not_applicable", reason="Requires agent routing or history")
                results.append(row)
                continue
            if args.mode == "agent" and digest is None:
                row.update(status="unavailable", reason="Configured Ollama model is unavailable",
            ...
            if args.mode == "retrieval":
                ...
                with tempfile.TemporaryDirectory() as directory:
                    ...
                    agent = Agent(index, reranker if config["rerank"] else None, model,
                                  ServiceCatalog(settings.index / "catalog.sqlite3"), memory,
                                  args.out.parent / (args.out.stem + "_traces"))
                    prior_results = [agent.ask(q, "evaluation", case["id"])
                ...
                row.update(answer=result["answer"], sources=sources, agent_status=result["status"],
            ...
            results.append(row)
    summaries = {config["name"]: summarize([r for r in results if r["configuration"] ==
                                            config["name"]]) for config in configs}
    deltas = {key: summaries["improved"][key] - summaries["baseline"][key]
              for key in ("recall_at_3", "reciprocal_rank", "correctness", "groundedness",
                          "relevance", "latency_ms")
              if summaries["improved"].get(key) is not None and
              summaries["baseline"].get(key) is not None}
    metadata = {
        "utc": datetime.now(UTC).isoformat(), "mode": args.mode, "judge": args.judge,
        "git_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip(),
        "working_tree_dirty": bool(subprocess.check_output(["git", "status", "--porcelain"])),
        "dataset_sha256": hashlib.sha256(cases_bytes).hexdigest(), "dataset_cases": len(cases),
        "index_sha256": hashlib.sha256((settings.index / "index.npz").read_bytes()).hexdigest(),
        "configuration": {k: str(v) for k, v in asdict(replace(settings, ollama_url="redacted")).items()},
        "generator_digest": digest, "judge_digest": judge_digest,
        "embedding_revision": getattr(index.encoder.model[0].auto_model.config, "_commit_hash", None),
        "reranker_revision": getattr(reranker.model.model.config, "_commit_hash", None),
        "python": platform.python_version(), "platform": platform.platform(),
        "packages": {p: importlib.metadata.version(p) for p in
                     ["langgraph", "rank-bm25", "sentence-transformers", "numpy", "torch"]},
        "experiments": configs,
    }
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps({"metadata": metadata, "summary": summaries,
                                   "improved_minus_baseline": deltas, "results": results}, indent=2)
                        + "\n")
